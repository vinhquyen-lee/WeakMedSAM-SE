...
def build_samus_vit_h(parent_classes, child_classes, checkpoint=None):
    return _build_samus(
...
def build_samus_vit_l(parent_classes, child_classes, checkpoint=None):
    return _build_samus(
        encoder_input_size=256,
...
def load_from2(
    samus, sam_dict, image_size, patch_size
):  # load the positional embedding
    samus_dict = samus.state_dict()
    dict_trained = {k: v for k, v in sam_dict.items() if k in samus_dict}
    token_size = int(image_size // patch_size)

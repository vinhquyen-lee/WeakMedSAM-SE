...
def get_point_prompt(
    ...
    if cam.dim() == 4:
        ...
        return torch.tensor(
            [
                get_point_prompt(cam[b, class_num], mask[b, 0], subtract, grid_ratio)
                for b in range(len(cam))
            ]
        ).unsqueeze(1)
    ...
    a = cam.size(0)
    grid_size = a // grid_ratio

    reshaped_cam = cam.view(grid_ratio, grid_size, grid_ratio, grid_size)

    block_sums = reshaped_cam.sum(dim=(1, 3))
    prompt_idx = torch.argmax(block_sums)
...
class Samus(nn.Module):
    ...
    def __init__(
        ...
        for b in self.image_encoder.blocks[-4:]:
    ...
    def forward_sam(
        ...
        _, image_embeddings = self.image_encoder(input_images)
    ...
    def forward(
        ...
        cls_embedding, _ = self.image_encoder(imgs)
        # _, cls_embedding = self.image_encoder(imgs)
        with torch.no_grad():
            # assert (
            #     False
            # ), f"\n\t{self.image_encoder.neck(cls_embedding).shape}\n\t{F.conv2d(cls_embedding, self.parent_fc.weight).shape}"
            # assert False, F.conv2d(cls_embedding, self.parent_fc.weight).shape
            cam_output = F.conv2d(cls_embedding, self.parent_fc.weight)
            # cam_output = F.relu(cam_output)
            cam_output = F.interpolate(cam_output, (256, 256), mode="bilinear")
            cam_output = max_norm(cam_output)
            # cam = (cam > 0.5).float()
    ...
    def forward_raw_mask(self, imgs: torch.Tensor, pt: torch.Tensor, lb: torch.Tensor):
        # pt: [b, n, 2]
        # lb: [b, n]
        _, seg_embedding = self.image_encoder(imgs)
        ...
        masks = F.interpolate(
            low_res_masks, imgs.shape[2:], mode="bilinear", align_corners=False
        )
        return masks

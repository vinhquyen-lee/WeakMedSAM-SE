...
class MaskDecoder(nn.Module):
    ...
    def predict_masks(
        ...
        # 1 iou + 1 total mask + 3 sub mask = (5, 256)
        output_tokens = torch.cat(
            [self.iou_token.weight, self.mask_tokens.weight], dim=0)
        output_tokens = output_tokens.unsqueeze(0).expand(
            sparse_prompt_embeddings.size(0), -1, -1)  # b 5 256
        # only cat with sparse_prompt # b 7 256 when sparse_prompt point is 2
        tokens = torch.cat((output_tokens, sparse_prompt_embeddings), dim=1)

        # Expand per-image data in batch direction to be per-mask
        if len(image_embeddings.shape) == 3:
            image_embeddings = image_embeddings.unsqueeze(0)
            # b 256 32 32 when the decoder operated in bs=1
        ...
        pos_src = torch.repeat_interleave(
            image_pe, tokens.shape[0], dim=0)  # b 256 32 32
        b, c, h, w = src.shape

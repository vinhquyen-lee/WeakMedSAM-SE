...
class ImageEncoderViT(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for blk in self.blocks:
            ...
            embeddings.append(x + cnn_ratio * cnnx)

        x = x + cnn_ratio * cnnx

        cls_embedding = x.permute(0, 3, 1, 2)

        seg_embedding = self.neck(x.permute(0, 3, 1, 2))

        return cls_embedding, seg_embedding
...
class ParaBlock(nn.Module):
    ...
    def __init__(
        ...
        if self.window_size == 0:
            # new to sam, MLP-adapter, no skip connection
            self.MLP_Adapter = Adapter(dim, skip_connect=False)
            self.Space_Adapter = qkvAttention(
    ...
    def forward(
        self, x: torch.Tensor, cnnx: torch.Tensor
